...
class DbusService:
  ...
  def _disconnect(self):
    ...
    if nodata > timedelta(seconds=null_timeout):
      ...
      self._VeDbus["/Pv/0/V"] = None
      self._VeDbus["/Pv/0/P"] = None
  ...
  def _update(self, state, loadcurrent, pvvoltage,
    ...
    try:
      ...
      self._VeDbus["/Pv/0/V"] = pvvoltage
      self._VeDbus["/Pv/0/P"] = pvpower
...
def _newservice(key, deviceinstance, productname, customname, phase, position, ip_address, hardware, firmware):
  ...
  self = DbusService(
    ...
    pathsReadWrite={
        ...
        "/Pv/0/V": {"initial": 0, "gettextcallback": lambda p, v: "{:0.1f}V".format(v)},
        "/Pv/0/P": {"initial": 0, "gettextcallback": lambda p, v: "{:0.0f}W".format(v)},
